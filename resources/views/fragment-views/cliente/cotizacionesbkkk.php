...
$("#btnImprimir").on('click', async function() {
    ...
    var diasVisita = parseInt($("#diasVisita").val());
    const batchSize = 5; // Ajusta el tamaño del lote según sea necesario

    if (!isNaN(numeroInicio) && !isNaN(numeroFin)) {
                // Manejar la impresión por números
                await imprimirPorNumeros(numeroInicio, numeroFin, batchSize);
            } else if (fechaInicio && fechaFin) {
                // Manejar la impresión por fechas
                await imprimirPorFechas(fechaInicio, fechaFin, batchSize);
            } else if (!isNaN(diasVisita)) {
                // Manejar la impresión por días de visita
                await imprimirPorVisitas(diasVisita, batchSize);
            } else {
                alert("Ingrese un rango de números, un rango de fechas o días de visita válidos.");
            }
        });
    ...
    try {
        for (let i = numeroInicio; i <= numeroFin; i += batchSize) {
            ...
            for (let j = i; j < i + batchSize && j <= numeroFin; j++) {
                promises.push(fetch(`/r/pedido/reporte/${j}`).then(response => {
                    if (!response.ok) {
                        throw new Error(`Pedido ${j} no encontrado.`);
                    }
                    return response.arrayBuffer();
                }));
            }
...
async function imprimirPorFechas(fechaInicio, fechaFin, batchSize) {
    const { PDFDocument } = PDFLib;
    const combinedPdf = await PDFDocument.create();
    ...
    try {
        ...
        while (currentDate <= endDate) {
            ...
            for (let i = 0; i < batchSize && currentDate <= endDate; i++) {
                promises.push(fetch(`/r/pedido/reporte/fecha/${formattedDate}`).then(response => {
                    if (!response.ok) {
                        throw new Error(`Pedido para la fecha ${formattedDate} no encontrado.`);
                    }
                    return response.arrayBuffer();
                }));
...
async function imprimirPorVisitas(diasVisita, batchSize) {
            const { PDFDocument } = PDFLib;
            const combinedPdf = await PDFDocument.create();
            ...
            try {
                // Aquí puedes implementar la lógica para manejar días de visita
                const response = await fetch(`/r/pedido/reporte/dias/${diasVisita}`);
                if (!response.ok) {
                    throw new Error(`No se encontraron pedidos para ${diasVisita} días de visita.`);
                }
                const pdfBuffer = await response.arrayBuffer();
                const pdf = await PDFDocument.load(pdfBuffer);
                const copiedPages = await combinedPdf.copyPages(pdf, pdf.getPageIndices());
