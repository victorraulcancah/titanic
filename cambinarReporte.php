...
")->fetch_assoc()['total'];
    ...
    public function consolidadoPedidosCamion()
    {
        ...
        if ($horario != "") {
            ...
            if ($horario == 'primer_corte') {
                $queryClientes .= " AND co.fecha_registro >= '$fechaSeleccionada 00:00:00' AND co.fecha_registro < DATE_ADD('$fechaSeleccionada 00:00:00', INTERVAL 32 HOUR) ";
            }
            if ($horario == 'segundo_corte') {
                $queryClientes .= " AND co.fecha_registro >= DATE_ADD('$fechaSeleccionada 00:00:00', INTERVAL 32 HOUR) AND co.fecha_registro < DATE_ADD('$fechaSeleccionada 00:00:00', INTERVAL 37 HOUR) ";
            }
            if ($horario == 'tercer_corte') {
                $queryClientes .= " AND co.fecha_registro >= DATE_ADD('$fechaSeleccionada 00:00:00', INTERVAL 37 HOUR) AND co.fecha_registro < DATE_ADD('$fechaSeleccionada 00:00:00', INTERVAL 48 HOUR) ";
            }
        }
        ...
        if (!empty($horario)) {
            $horarioLabels = ['todos' => 'Todos', 'diurno' => 'De 8:00 a.m. a 3:00 p.m.', 'nocturno' => 'De 3:00 p.m. a 7:59 a.m.', 'primer_corte' => 'Primer Corte (0 a 32 horas)', 'segundo_corte' => 'Segundo Corte (32 a 37 horas)', 'tercer_corte' => 'Tercer Corte (37 a 48 horas)'];
    ...
    public function consolidadoTotalPedidosCamion()
    {
        ...
        $html = "<h2 style='text-align:center; margin-bottom:6px;'>Consolidado Total - Todos los Camiones y Mercados</h2>";
        $html .= " <div style='width: 100%;overflow: hidden;clear: both;'>";
        if (!empty($fechaSeleccionada) && !empty($fechaFinSeleccionada)) {
            $html .= "<p style=''>Periodo: {$fechaSeleccionada}  al  {$fechaFinSeleccionada}</p>";
        }
        ...
        foreach ($mercados as $merc) {
            ...
            AND co.sucursal='{$_SESSION['sucursal']}'
            AND co.estado!=2 " . $queryClientes . $concatmerc . "
            ORDER BY c.datos ASC, co.numero ASC
            ";
            ...
            $query_productos .= " GROUP BY p.codigo, p.descripcion, pc.presenta_cnt, pc.medida ORDER BY p.descripcion ASC";
            ...
            foreach ($listaProd1 as $prod) {
                ...
                $rowHTML .= "
                <tr>
                    <td style='width:30px; font-weight:bold; color:#000; font-family:Arial,sans-serif; font-size:11px; text-align:center; border-left:1px solid #fff; padding:2px; white-space:nowrap;'>$contador</td>
                    <td style='width:65px; font-weight:bold; color:#000; font-family:Arial,sans-serif; font-size:11px; text-align:center; border-left:1px solid #fff; padding:2px; white-space:nowrap;'>{$prod['codigo']}</td>
                    <td style='font-weight:bold; color:#000; font-family:Arial,sans-serif; font-size:11px; text-align:left; border-left:1px solid #fff; padding:2px;'>{$prod['descripcion']}</td>
                    <td style='width:50px; font-weight:bold; color:#000; font-family:Arial,sans-serif; font-size:11px; text-align:center; border-left:1px solid #fff; padding:2px; white-space:nowrap;'>{$prod['total_medida']}</td>
                    <td style='width:50px; font-weight:bold; color:#000; font-family:Arial,sans-serif; font-size:11px; text-align:center; border-left:1px solid #fff; padding:2px; white-space:nowrap;'>{$prod['total_cantidad']}</td>
                </tr>
                ";
                $contador++;
            }

            if ($total_consolidado == 0) continue;

            $html .= "<h3 style='margin-top:20px; margin-bottom:4px;'>Mercado: {$mercadito}</h3>";
            $html .= "<div style='width:100%; padding-top:5px;'>
            <table style='width:100%; border-collapse:collapse;'>
                <tr>
                    <td style='width:30px; font-size:12px; text-align:center; color:#000; border:1px solid #fff; padding:2px; white-space:nowrap;'><strong>Item</strong></td>
                    <td style='width:65px; font-size:12px; text-align:center; color:#000; border:1px solid #fff; padding:2px; white-space:nowrap;'><strong>Código</strong></td>
                    <td style='font-size:12px; text-align:left; color:#000; border:1px solid #fff; padding:2px;'><strong>PRODUCTO</strong></td>
                    <td style='width:50px; font-size:12px; text-align:center; color:#000; border:1px solid #fff; padding:2px; white-space:nowrap;'><strong>MEDIDA</strong></td>
                    <td style='width:50px; font-size:12px; text-align:center; color:#000; border:1px solid #fff; padding:2px; white-space:nowrap;'><strong>CANTIDAD</strong></td>
                </tr>
                $rowHTML
                <tr>
                    <td colspan='3'></td>
                    <td style='font-size:11px; text-align:right; font-weight:bold; border-top:2px solid #000; padding:2px;'>Total</td>
                    <td style='font-size:11px; text-align:center; font-weight:bold; border-top:2px solid #000; padding:2px;'>{$total_consolidado}</td>
                </tr>
            </table>
            </div>";
        }
